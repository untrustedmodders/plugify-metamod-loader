...
#define BASE_PATH PLUGIFY_PATH_LITERAL("" MMS2_GAME_NAME "/" "addons" "/" "plugify" "/")
#define MAMBA_PATH BASE_PATH PLUGIFY_PATH_LITERAL("bin" "/" MMS2_BINARY "/" MMS2_EXECUTABLE_PREFIX "micromamba" MMS2_EXECUTABLE_SUFFIX)
#if MMS2_PLATFORM_WINDOWS
#define SERVER_PATH PLUGIFY_PATH_LITERAL("" MMS2_GAME_NAME "/" "bin" "/" MMS2_BINARY "/" MMS2_LIBRARY_PREFIX "server" MMS2_LIBRARY_SUFFIX)
#else
#define SERVER_PATH PLUGIFY_PATH_LITERAL("bin" "/" MMS2_BINARY "/../../" MMS2_GAME_NAME "/" "bin" "/" MMS2_BINARY "//" MMS2_LIBRARY_PREFIX "server" MMS2_LIBRARY_SUFFIX) // Metamod hack
#endif
...
DynLibUtils::CVTFHookAuto<&IGameSystem::ServerPostSimulate> s_ServerGamePostSimulate;

void ServerGamePostSimulate(IGameSystem* pThis, const EventServerPostSimulate_t& msg) {
...
class PlugifyPlugin : public ISmmPlugin {
	static Result<void> InstallServerHooks(const fs::path& serverPath) {
		std::error_code ec;
		if (!fs::exists(serverPath, ec)) {
			return MakeError("Server library not found at: {}", plg::as_string(serverPath));
		}
#if MMS2_PLATFORM_WINDOWS
		...
		server.LoadFromPath(plg::as_string(serverPath), flags);
		if (!server) {
	...
	static Result<std::shared_ptr<Plugify>> InitializePlugify() {
		// Build base directory path
		fs::path gameDir(Plat_GetGameDirectory());
		fs::path baseDir = gameDir / BASE_PATH;
		fs::path exePath = gameDir / MAMBA_PATH;
		fs::path serverPath = gameDir / SERVER_PATH;

		// Install server hooks
		if (auto hookResult = InstallServerHooks(serverPath); !hookResult) {
...
public:// ISmmPlugin
	...
	const char* GetDescription() override {
		return MMS2_PROJECT_DESCRIPTION;
	}

	const char* GetName() override {
		return MMS2_PROJECT_NAME;
	}

	const char* GetURL() override {
		return MMS2_PROJECT_HOMEPAGE_URL;
	}
} g_Plugin;
PL_EXPOSURE_FUNC(PlugifyPlugin, g_Plugin)
